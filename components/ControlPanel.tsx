...
import { requestPresignedUrl, uploadFileToR2, reorderMedia, toggleMediaVisibility, deleteMedia } from '../services/apiService';

interface Props {
  schemeIds: SchemeId[];
  schemes: Record<SchemeId, MediaItem[]>;
  schemeNames: Record<SchemeId, string>;
  addScheme: (name: string) => Promise<string | null>;
  renameScheme: (oldId: SchemeId, newName: string) => void;
  ...
  setPinnedMediaId: (id: string | null) => void;
...
const ControlPanel: React.FC<Props> = ({
  ...
  const handleAddSchemeConfirm = async (e: React.FormEvent | React.MouseEvent) => {
    e.preventDefault();
    if (newSchemeInput.trim()) {
      const newId = await addScheme(newSchemeInput.trim());
      if (newId) setEditingScheme(newId);
      setNewSchemeInput('');
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      // For each file: request presigned URL → upload to R2 → collect mediaItem
      const newMediaItems: MediaItem[] = [];

      for (const file of fileArray) {
        console.log('[Upload] Step 1: Requesting presigned URL for', file.name);
        const presigned = await requestPresignedUrl(editingScheme, {
          fileName: file.name,
          contentType: file.type || 'application/octet-stream',
          fileSize: file.size,
        });
        console.log('[Upload] Step 1 done, got uploadUrl:', presigned.uploadUrl);

        console.log('[Upload] Step 2: Uploading to R2...');
        await uploadFileToR2(presigned.uploadUrl, file);
        console.log('[Upload] Step 2 done, file uploaded successfully');
  ...
  const executeDeleteMedia = async (id: string, e: React.MouseEvent) => {
    ...
    if (id === pinnedMediaId) setPinnedMediaId(null);
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between border-b border-white/10 pb-6 flex-shrink-0 gap-4">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                ...
                <div key={id} className="relative group/tab">
                  {renamingId === id ? (
                    ...
                    <div className="flex items-center">
                      ...
                      >
                        <span className="text-sm sm:text-base">{schemeNames[id] || id}</span>
                      ...
                      <div className={`absolute -top-1 right-0 flex gap-1 opacity-0 group-hover/tab:opacity-100 transition-opacity z-10 scale-75 ${editingScheme === id ? 'translate-x-1' : ''}`}>
                         <button
                           onClick={(e) => { e.stopPropagation(); setRenamingId(id); setRenameInput(schemeNames[id] || id); }}
            ...
            <label className={`bg-[#E11D48] text-white px-6 md:px-8 py-2.5 md:py-3.5 rounded-full font-bold transition-all flex items-center gap-2 shadow-[0_0_20px_rgba(225,29,72,0.3)] flex-shrink-0 text-xs sm:text-sm ${isUploading ? 'opacity-60 cursor-wait' : 'cursor-pointer hover:scale-105'}`}>
              ...
              {isUploading ? '上傳中...' : `上傳至 ${schemeNames[editingScheme] || editingScheme}`}
        ...
        <div className="flex flex-col gap-4 flex-1 min-h-0 bg-white/[0.02] border border-white/5 rounded-[1.5rem] md:rounded-[2.5rem] p-4 md:p-8 overflow-hidden shadow-inner">
          <div className="flex items-center justify-between flex-shrink-0">
             <div className="flex flex-col gap-1">
               <h3 className="text-xs md:text-sm font-bold tracking-[0.2em] text-white uppercase">方案 {schemeNames[editingScheme] || editingScheme} 素材庫</h3>
          ...
          <div className="flex-1 overflow-y-auto pr-1 md:pr-2 custom-scrollbar">
            {currentMediaList.length > 0 ? (
                ...
                {currentMediaList.map((media, idx) => (
                  <div
                    ...
                    className={`relative aspect-[16/10] rounded-2xl md:rounded-3xl overflow-hidden glass-card group border-2 transition-all duration-300 cursor-grab active:cursor-grabbing hover:scale-[1.02] ${
                      draggedIndex === idx
                        ? 'opacity-20 scale-95 border-dashed border-white/50'
                        : pinnedMediaId === media.id
                          ? 'opacity-100 border-[#E11D48]/60 shadow-xl shadow-[#E11D48]/20'
                          : 'opacity-100 border-white/5 shadow-xl'
                    }`}
                  >
                    ...
                    <div className="absolute inset-0 bg-black/80 opacity-0 group-hover:opacity-100 transition-all duration-300 flex items-center justify-center gap-3 z-30">
                      {confirmDeleteId === media.id ? (
                        ...
                        <>
                          {editingScheme === liveScheme && (
                            <button
                              onMouseDown={(e) => e.stopPropagation()}
                              onClick={(e) => { e.preventDefault(); e.stopPropagation(); setPinnedMediaId(pinnedMediaId === media.id ? null : media.id); }}
                              className={`p-2.5 md:p-3 rounded-xl md:rounded-2xl transition-all hover:scale-110 ${
                                pinnedMediaId === media.id ? 'bg-[#E11D48] text-white shadow-[0_0_10px_rgba(225,29,72,0.4)]' : 'bg-white/10 text-white'
                              }`}
                            >
                              <Pin size={18} />
                            </button>
                    ...
                    {pinnedMediaId === media.id && (
                      <div className="absolute top-2.5 right-2.5 z-10 bg-[#E11D48] text-white px-2 py-0.5 rounded-lg text-[9px] font-bold flex items-center gap-1 shadow-lg">
                        <Pin size={8} /> 釘選中
                      </div>
                    )}
              ...
              <div className="h-full min-h-[250px] flex flex-col items-center justify-center opacity-20 border-2 border-dashed border-white/10 rounded-[1.5rem] md:rounded-[2rem]">
                ...
                <p className="font-bold tracking-[0.2em] md:tracking-[0.3em] uppercase text-xs">方案 {schemeNames[editingScheme] || editingScheme} 尚未上傳素材</p>
              </div>
            )}
          </div>
        </div>
          ...
          <div className="glass-card p-8 md:p-12 rounded-[2rem] md:rounded-[3rem] border-red-500/30 flex flex-col items-center gap-6 md:gap-8 max-w-md w-full animate-in zoom-in duration-300">
            ...
            <div className="text-center">
              ...
              <p className="text-white/40 text-xs md:text-sm leading-relaxed">
                您即將刪除「<span className="text-white font-bold">{schemeNames[confirmSchemeDeleteId] || confirmSchemeDeleteId}</span>」。<br/>
                此操作將永久移除該方案內容，且無法復原。
              </p>
            </div>
